...
filter(subgroup=="Voters")%>%
  ...
  mutate(date=mdy(enddate),week=week(date),year=year(date),net_approval=approve-disapprove)%>%
  group_by(week,year)%>%
...
ggplot(Trump_approval,aes(x=week))+
  geom_line(aes(y=lower_bound))+geom_line(aes(y=mean))+ 
  geom_line(aes(y=upper_bound))+geom_point(aes(y=mean))+
  #Fill the confidence interval with colour
  geom_ribbon(aes(ymin=lower_bound,ymax=upper_bound),alpha=0.2)+facet_wrap(~year)+
...
",subtitle="Weekly average of all polls",x="Week of the year",y="Average Net Approval (%)")+
  #Customize the size of the font and scales
  theme(text = element_text(size = 20),legend.position = "none") + 
  scale_y_continuous(limits=c(-20, 7.5),n.breaks=12)+ scale_x_continuous(limits=c(0, 53),n.breaks=5)


```

We can observe that in the top leftmost plot, the CI becomes very wide and this is due to an increase in t_critical caused by a very small sample size (2 polls only). For the sake of reference, we can compare our plots to the ones below and observe that the values and trends match.
